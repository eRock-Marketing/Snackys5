...
<div id="consent-manager" data-nosnippet class="d-none" tabindex="-1">
	...
	{block name='snippets-consent-manager-button'}
		<button type="button" class="consent-btn consent-btn-outline-primary" id="consent-settings-btn" aria-label="{lang key='cookieSettings' section='consent'}">
					...
					{block name='snippets-consent-manager-banner-body-description'}
						<div class="consent-banner-description">
							...
							{block name='snippets-consent-manager-banner-body-description-description'}
								...
								{/if}
								{if $privacyUrl !== '#'}
									<a href="{$privacyUrl}">{$privacyName}</a>
								{/if}
							{/block}
						</div>
					{/block}
								...
								{if $item->hasMoreInfo()}
									<div class="consent-help consent-more-description consent-hidden" id="consent-{$id}-description">
										...
										{if !empty({$item->getPrivacyPolicy()})}
											...
											<a href="{$item->getPrivacyPolicy()}" target="_blank" rel="noopener">{$item->getPrivacyPolicy()|truncate:100:"[...]":true}</a>
										{/if}
									</div>
								{/if}
							{/block}
			...
			{block name='snippets-consent-manager-confirm-content'}
				<div class="consent-modal-content">
					{block name='snippets-consent-manager-confirm-close'}
						<button type="button" class="consent-modal-close" data-toggle="consent-close" aria-label="{lang key='closeConsent' section='custom'}">
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path fill="currentColor" d="M207.6 256l107.72-107.72c6.23-6.23 6.23-16.34 0-22.58l-25.03-25.03c-6.23-6.23-16.34-6.23-22.58 0L160 208.4 52.28 100.68c-6.23-6.23-16.34-6.23-22.58 0L4.68 125.7c-6.23 6.23-6.23 16.34 0 22.58L112.4 256 4.68 363.72c-6.23 6.23-6.23 16.34 0 22.58l25.03 25.03c6.23 6.23 16.34 6.23 22.58 0L160 303.6l107.72 107.72c6.23 6.23 16.34 6.23 22.58 0l25.03-25.03c6.23-6.23 6.23-16.34 0-22.58L207.6 256z"/></svg>
						</button>
					{/block}
